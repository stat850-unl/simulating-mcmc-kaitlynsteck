...
simulation <- data.frame(distance = replicate(1000, single_uncon(582)))

dist_uncon <-ggplot(simulation, aes(x=distance))+
geom_histogram(binwidth = .75, fill= "pink", color="hot pink")
...
Yes, it can be vectorized by the code: df<- data.frame(x,y,z) df2<- colSums(df) which is placing the data into a vector. 
...
single_con <- function(n){
  theta <- runif(n, min= 0, max= 2*pi)
  phi <- runif(n, min= 0, max=pi)
  phi2<- lag(phi)
  diff<- abs(phi-phi2)
  angle <- runif(diff, min=pi/4, max=pi)
...
The histogram has become more normalized. The previous unconstrained was slightly left skewed, but the constrained is a bit more normalized to the center.  

```{r}
set.seed(235647820)

simulation2 <- data.frame(distance = replicate(1000, single_con(582)))

dist_con <-ggplot(simulation2, aes(x=distance))+
geom_histogram(binwidth = .75, fill= "hot pink", color= "pink")
